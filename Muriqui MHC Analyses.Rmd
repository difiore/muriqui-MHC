...
library(ggpubr) # for regression equations on plots
library(DescTools) # KendallTauB
```

```{r Load and Process Genotype Data}
# Genotype data is held in two files, one with adult genotypes and one with all genotypes
...
# Set the random seed. We use the same random seed for each simulation to generate results for exact values reported in paper, but the seed can be anything! P values will change slightly, but results are unchanged...
seed <- 11539 
```
...
adults_pairwise_R <- adults_rel$relatedness %>%
  ...
  mutate(pair = paste0(ind1.id, "=", ind2.id)) # 2016 dyads... this is a half matrix

all_pairwise_R <- all_rel$relatedness %>%
  select(pair.no, ind1.id, ind2.id, quellergt) %>%
  mutate(pair = paste0(ind1.id, "=", ind2.id)) # 6105 dyads... this is a half matrix

# We want to keep these structures as half matrices to use in our simulations to test predictions 1, 2a, 2b, and 3a so as no not artificially double the the number of dyads in any dyad type category
```
...
# Using the "adult_pairwise_R" half matrix, which is in long format and includes the Queller-Goodnight estimated R as calculated by {related}, we add in information on sex of each adult, and then assign dyad.type as MM (male-male), FF (female-female), or MS (mixed sex)
...
# NOTE: Visualizing this plot will give some warnings about missing values for geom_vline() and geom_rect()... this is fine, just a result of how the summary table for adding critical values regions is structured
```
...
We now want to create several data matrices we will need to test hypotheses.
...
Finally, we also create a matrix of male "uniqueness" relative to each female, i.e., the total number of alleles a male has that each female does not, as follows...
...
# This is "overview" matrix now contains information on whether each male was considered a candidate sire or not for each of the 47 offspring in the dataset, with dam ID as the rowname and candidate sire ID as the column name
```
...
## P2a and P2b - MHC DISSMILARITY and UNIQUENESS
...
As above, we extract observed uniqueness among actual sires relative to dams by pulling the relevant data from the original "male.uniqueness" matrix and calculating the average of these values...
...
(p.value <- sum(sim.male.uniqueness$stat > mean.male.uniqueness)/reps)
```
...
male.uniqueness.plot <- ggplot(
  ...
  annotate("rect", xmin = -Inf, xmax = male.uniqueness.crit.val, ymin = 0, ymax = Inf, alpha = 0.25, fill = "grey")
...
## Comparing Mean Dam-Sire R to Mean R among Kinship Classes
...
```{r Calculate P Values for Dam-Sire to Kinship Class Comparison}
...
Additional kinship analyses use KINGROUP2's implementation of the Queller-Goodnight R estimate, which is slightly different than that calculated by {related}. In the analyses below, involving the comparisons of the number of inferred close kin for each individual as explored using KINSGROUP2, we use the KINGROUP2 estimator of R because we are also looking at the results of KINGROUP2's likelihood ratio tests. Nonetheless, the correlation between {related} and KINGROUP2's estimates of dyadic R are extremly similar, which we show initially.
...
```{r Comparisons of Dam-Sire Dyads to Kinship Classes and Adult Dyad Types}
...
# And, finally, we can compare among same- and mixed-sex adult dyads as we did previously with the adults only estimated R values from {related}...
...
# We then get the estimated R for these dyads based on the {related} algorithm by merging these with "adults_pairwise_R", but first we need to convert "adults_pairwise_R" to a full matrix in order to be able to look up any dyad. To do this, we make a "second half" for the matrix in long format and bind it to the first.
 
adults_pairwise_R_second_half <-
  adults_pairwise_R %>%
  mutate(
    ind1.id.new = ind2.id,
    ind2.id = ind1.id,
...
adults_pairwise_R_full <- bind_rows(
  adults_pairwise_R,
  adults_pairwise_R_second_half) %>% select(-c(pair.no))
...
# The dataframe "adults" contains information on individual heterozygosity (proportion of typed loci that are heterozygous) and number of MHC alleles.
...
# load in summary of dissimilarity among all pairs and merge with pairwise estimated R from {related}
r_and_diss <- read_tsv("data/mhc_dissimiliarity.txt", col_names = TRUE) %>%
  left_join(., adults_pairwise_R_full, by = c("ind1.id"="ind1.id", "ind2.id"="ind2.id", "pair"="pair"))

cor.test(r_and_diss$quellergt, r_and_diss$mhc.dissimilarity, method = "kendall")

KendallTauB(r_and_diss$quellergt, r_and_diss$mhc.dissimilarity, conf.level = 0.95)

p <- ggplot(data = r_and_diss, aes(x=quellergt, y= mhc.dissimilarity)) + geom_point() + geom_smooth (method = "lm", se=TRUE)

p <- ggplot(data = r_and_diss, aes(x=as.factor(mhc.dissimilarity), y= quellergt)) + geom_boxplot()

m <- lm(mhc.dissimilarity ~ quellergt, data = r_and_diss)
summary(m)
```
